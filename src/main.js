...
${Object.keys(CUSTOM_CONFIG()).length?(()=>{const firstId=Object.keys(CUSTOM_CONFIG())[0];const cfg=CUSTOM_CONFIG()[firstId];return `<section class="builderPromoWrap"><button class="builderPromo" data-open-builder="${firstId}"><div class="builderPromoText"><span class="builderPromoIcon">${icon('chef')}</span><i class="builderPromoTag">MA RECETTE</i><h3>Envie de créer votre recette ?</h3><p>Composez votre plat pas à pas, choisissez vos ingrédients et faites-vous plaisir !</p><em>${icon('chef')} Créer ma recette ${icon('arrow-right')}</em></div><div class="builderPromoImg"><img src="${cfg.img}" alt=""><span class="builderPromoNote">Votre recette,<br>votre style !</span></div></button></section>`})():''}
