...
<button data-go="menu" class="${['menu','category'].includes(S.route)?'on':''}"><i>${icon('menu')}</i><small>Menu</small></button>
...
<section class="perks"><div><b>${icon('fire')}</b><strong>Cuisson parfaite</strong><small>Doré & croustillant</small></div><div><b>${icon('check')}</b><strong>Ingrédients frais</strong><small>Sélectionnés avec soin</small></div><div><b>${icon('delivery')}</b><strong>Livraison rapide</strong><small>30–45 min</small></div></section>
...
function menu(){
 ...
 return `<section class="menuTop"><button class="back" data-go="home">${icon('arrow-left')}</button><div class="crumb">Accueil　›　${S.type==='pizza'?'Pizza':'Pannuezo'}</div><div class="menuHero"><div><i>${S.type==='pizza'?'PIZZA':'PANNUEZO'}</i><h1>${S.type==='pizza'?'PIZZA':'PANNUEZO'}</h1><span class="uline"></span><p>${S.type==='pizza'?'Pizzas artisanales cuites à la perfection avec des ingrédients frais et une pâte maison moelleuse et croustillante.':'Découvrez nos Pannuezo ultra fondants, généreusement garnis et préparés avec des ingrédients de qualité.'}</p></div><div class="menuHeroImg"><img src="${S.type==='pizza'?'/images/pizza-card.jpg':'/images/pannuezo-card.jpg'}"></div></div>
<div class="featureRow"><span>${icon('fire')} <b>Cuisson ${S.type==='pizza'?'au four':'parfaite'}</b><small>${S.type==='pizza'?'haute température':'Doré & croustillant'}</small></span><span>${icon('check')} <b>Ingrédients frais</b><small>Sélectionnés avec soin</small></span><span>${icon('clock')} <b>Livraison rapide</b><small>30–45 min</small></span></div></section>
...
function sticky(){return S.cart.length?`<div class=sticky><span><b>${count()} articles</b><small>${formatPrice(total())}</small></span><button class=cta data-go=cart>VOIR LE PANIER ${icon('arrow-right')}</button></div>`:''}
