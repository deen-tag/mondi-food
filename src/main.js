...
function sticky(){return S.cart.length?`<div class=sticky><span class="stickyBag">${icon('cart')}<b>${count()}</b></span><span class="stickyInfo"><b>${count()} articles</b><small>${formatPrice(total())}</small></span><button class="cta pill" data-go=cart>VOIR LE PANIER ${icon('arrow-right')}</button></div>`:''}
