...
// Paiement en ligne : activé par défaut, désactivable depuis l'admin (Réglages).
// Tant que le catalogue n'est pas chargé, on considère qu'il est activé (état par
// défaut) pour éviter un flash "paiement à la livraison uniquement" au premier rendu.
const onlinePaymentEnabled=()=>!CATALOG||CATALOG.settings?.onlinePaymentEnabled!==false;
...
<section class="block tight"><div class="heading"><span><i>À LA CARTE</i><h2>Nos incontournables</h2></span><button data-go="menu">Tout voir ${icon('arrow-right')}</button></div><div class="railWrap"><div class="rail">${featuredProducts().map(mini).join('')}</div></div></section>
...
<section class="block tight catBlock"><div class="heading"><span><i>PARCOURIR</i><h2>Nos catégories</h2></span><button data-go="menu">Tout voir ${icon('arrow-right')}</button></div><div class="homeTiles catGrid" id="categories">
${mainCategories().map(c=>tile(c.slug,typeMeta(c.slug).heroImg,c.label.toUpperCase(),`Découvre notre sélection ${c.label.toLowerCase()}`)).join('')}</div></section>
<section class="perks perksBar"><div><b>${icon('fire','',true)}</b><strong>Cuisson parfaite</strong><small>Doré & croustillant</small></div><div><b>${icon('check')}</b><strong>Ingrédients frais</strong><small>Sélectionnés avec soin</small></div><div><b>${icon('delivery')}</b><strong>Livraison rapide</strong><small>30–45 min</small></div></section>
...
function bind(){
 ...
 document.querySelector('#order')?.addEventListener('submit',async e=>{
  ...
  // Filet de sécurité côté client : si le paiement en ligne a été désactivé
  // entre-temps (ou en cas de manipulation du formulaire), on retombe sur
  // "à la livraison" plutôt que de tenter un paiement Stripe refusé par le serveur.
  if(!onlinePaymentEnabled())d.payment='delivery';
  const t=finalTotal();
