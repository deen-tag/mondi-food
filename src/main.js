...
function menu(){
 ...
 return `<section class="menuTop"><button class="back" data-go="home">${icon('arrow-left')}</button><div class="crumb"><span class="crumbHome">Accueil</span><span class="crumbSep">›</span><span class="crumbCurrent">${S.type==='pizza'?'Pizza':'Pannuezo'}</span></div><div class="menuHero"><div><i>${S.type==='pizza'?'PIZZA':'PANNUEZO'}</i><h1>${S.type==='pizza'?'PIZZA':'PANNUEZO'}</h1><span class="uline"></span><p>${S.type==='pizza'?'Pizzas artisanales cuites à la perfection avec des ingrédients frais et une pâte maison moelleuse et croustillante.':'Découvrez nos Pannuezo ultra fondants, généreusement garnis et préparés avec des ingrédients de qualité.'}</p></div><div class="menuHeroImg"><img src="${S.type==='pizza'?'/images/pizza-card.jpg':'/images/pannuezo-card.jpg'}"></div></div>
